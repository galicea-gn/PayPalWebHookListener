...
namespace RevolutionGolf.EventRequestBroker.Middleware
{
    public class EventRequestAuthenticationService : IEventRequestAuthenticationService
    {
        public async Task<bool> IsValidRequest(HeaderObject headerObject, string body, string webHookId)
        {
            ...
            try
            {
                string hashAlgorithm = ConvertAuthAlgorithmHeaderToHashAlgorithmName(headerObject.AuthenticationAlogrithm);
                ...
                var x509CertificateCollection = await CertificateManager.Instance.GetCertificatesFromUrl(headerObject.CertificateUrl);
                var trustedX509Certificate = CertificateManager.Instance.GetTrustedCertificateFromFile();
                ...
                if (isValid)
                {
                    ...
                    var signatureBytes = Convert.FromBase64String(headerObject.TransmissionSig);
        ...
        private string ConvertAuthAlgorithmHeaderToHashAlgorithmName(string authAlgorithmHeader)
        {
    ...
    internal sealed class CertificateManager
    {
        ...
        public async Task<X509Certificate2Collection> GetCertificatesFromUrl(string certUrl)
        {
        ...
        public X509Certificate2 GetTrustedCertificateFromFile()
        {
            try
            {
                return new X509Certificate2(File.ReadAllBytes(EventRequestAuthenticationDefaults.TrustedFileCertLocation));
            }
            catch (Exception ex)
            {
                throw new Exception($"Unable to load trusted certificate.", ex);
            }
        }
